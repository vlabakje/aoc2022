class Map():
    ...
    def dijkstra(self):
        ...
        shortest_path[self.end] = 0
        shortest_path[self.start] = 2**32
        previous_nodes = {}
        while len(unvisited_nodes):
            lowest = tuple(self.start)
            for node in unvisited_nodes:
                if shortest_path[node] < shortest_path[lowest]:
                    lowest = node
            if self.nodes[lowest] == 0:
                return shortest_path[lowest]
            #print(f"{len(unvisited_nodes)=} {lowest=} {shortest_path[lowest]=}")
            #print(f"{list(self.neighbours(lowest))=}")
